...
import { getCategoryHub, guidesInCategory } from "@/lib/guides";
...
// dynamique voisine `/guides/[slug]`. Contenu éditorial unique (intro + FAQ +
// grille) pour une page à part entière, pleinement indexable.
const HUB_SLUG = "par-metier";
const hub = getCategoryHub(HUB_SLUG)!;
...
export default function ParMetierHubPage() {
  ...
  const schema = graph(
    breadcrumbSchema([
      ...
      { name: hub.h1, path: `/guides/${hub.slug}` },
    ]),
    {
      "@type": "CollectionPage",
      name: hub.h1,
        ...
        <section className="relative isolate overflow-hidden border-b border-line">
          ...
          <Container className="py-16 sm:py-24">
            <nav aria-label="Fil d'Ariane" className="text-sm text-muted">
              ...
              <span className="mx-2 text-line">/</span>
              <span className="text-ink-soft">Par métier</span>
            </nav>
        ...
        <section className="border-t border-line bg-surface">
          <Container className="flex flex-col items-center gap-4 py-14 text-center">
            ...
            <p className="max-w-md text-[15px] text-ink-soft">
              Le présentoir reviu met la méthode de votre métier sur votre
