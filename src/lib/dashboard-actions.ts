...
export async function claimStandAction(
  ...
  const supabase = await createSupabaseServer();
  // Codes générés en minuscules, PIN en majuscules : on normalise la saisie
  // pour tolérer la casse relevée sur le présentoir.
  ...
  if (error) {
    const map: Record<string, string> = {
      ...
      invalid_pin: "PIN incorrect. Vérifiez le code PIN inscrit sur le présentoir.",
    };
